...
from caencamac import OP_BLKFA
...
def runCAMAC(configModule, maxEvents, maxTimeSec,
    ...
    try:
        ...
        for eventNumber in range(numEventsLimit):
            ...
            h.BLKBUFFS(tdc_channels)
            for slot in tdc_slots:
                tdcValues = h.receiveBlock(OP_BLKFA, 2, slot, 0, tdc_channels, 10000)
            ...
            h.BLKBUFFS(adc_channels)
            for slot in adc_slots:
                adcValues = h.receiveBlock(OP_BLKFA, 2, slot, 0, adc_channels, 10000)
