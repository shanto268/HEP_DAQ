...
short CROPEN(char *address)
{
	...
	crate_info[crate_id].irq_comm_count = 0;
...
short CRCLOSE(short crate_id)
{
	...
	if (crate_info[crate_id].connected) {
        ...
        if (crate_info[crate_id].irq_comm_count)
...
short getCRTOUT(short crate_id, unsigned int *tout)
{
	...
	*tout = crate_info[crate_id].tout_ticks;
...
short CMDR(short crate_id, char *resp, int size)
{
	...
	if (crate_info[crate_id].tout_ticks == 0)
		res = csock_recvline(crate_info[crate_id].sock_ascii, resp, size);
	else
        {
		res = csock_recvline_t(crate_info[crate_id].sock_ascii, resp, size, crate_info[crate_id].tout_ticks);
                if (res == -3)
...
short waitForIRQ(short crate_id, int ackImmediately, int *code, int *value)
{
    ...
    *value = 0;
    ...
    if (res > 0)
    {
        unsigned long irq_data = 0;

        crate_info[crate_id].irq_comm_count++;
...
short ackIRQ(short crate_id)
{
    ...
    if (crate_info[crate_id].irq_comm_count != 1)
        return CRATE_PROTOCOL_ERROR;
