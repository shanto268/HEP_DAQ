...
class TDCAnalyzer(DummyModule):
    ...
    def processEvent(self, runNumber, eventNumber, eventRecord):
        ...
        tdcCalc = dict()
        #print("len TDC is ", lenTDCData)
        
        """
        if lenTDCData < 2:
             print("it is up")
             tdcCalc.update({"Layer1diff":np.nan})
             tdcCalc.update({"Layer1asym":np.nan})
             tdcCalc.update({"Layer2diff":np.nan})
             tdcCalc.update({"Layer2asym":np.nan})
        """
        ...
        #ak
        """
        elif (2,0) not in tdcData or (2,1) not in tdcData and lenTDCData >= 2:
            tdcCalc.update({"Layer1diff":0})
            tdcCalc.update({"Layer1asym":0})

        elif (2,3) not in tdcData or (2,4) not in tdcData and lenTDCData >= 2:
